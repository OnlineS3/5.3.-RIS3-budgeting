 ...
 */ 
 $newcsvfile  = str_replace(".xlsx",".csv",$filename);
 $newcsvfile  = str_replace(".xls",".csv",$filename);
 $newcsvfile = str_replace(" ","-",$newcsvfile);
 $newcsvfile = $excel2csv_dir . '/' . $newcsvfile;
 if(!is_dir('bin')) {mkdir("bin", 0770);}; 
 //if(!is_dir('csv')) {mkdir("csv", 0777);};
 
 echo ' xls_file ' . $filename;
 echo ' csv_file ' . $newcsvfile;
