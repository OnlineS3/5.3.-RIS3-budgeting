...
function in2rp($conn) {
	...
	$imp_sel = "SELECT in_data.description, reg.code as code, in_data.var_id, reg.code FROM in_data JOIN region reg ON in_data.region = reg.label";
	...
	for ($i=0; $i<count($year_array); $i++) {
		...
		$sql_base = "INSERT INTO rp_data (var_id, code, year_id, value)
                                SELECT DISTINCT rt.var_id, rt.code, '$year_array[$i]', ind.$year
